...
public final class Utilities {
	...
	public static List<String> getContainsConditions(String cond) {
		...
		String[] condNoQuote = cond.split("\"+([^\"]*)+\"");
		for(String cad : condNoQuote)
			lCond = lCond.replaceAll(cad, "");

		String [] condQuote = lCond.split("\"+");

		for(String cad : condNoQuote) {
			String [] subCond = cad.split("\\s+");
			for(String elem : subCond)
				if(!elem.trim().equals(""))
					conditions.add(elem.trim());
		}

		for(String cad : condQuote) {
			if(!cad.equals(""))
				conditions.add(cad);
		}
